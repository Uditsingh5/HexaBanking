...
if (!transporter) {
  console.warn(
    '[Email] Disabled — set EMAIL_USER plus either EMAIL_APP_PASSWORD (simplest) or OAuth tokens (CLIENT_ID, CLIENT_SECRET, REFRESH_TOKEN) in server/.env'
  );
} else {
  transporter.verify((error) => {
    if (error) {
      const reason = error.response || error.message || 'unknown auth error';
      console.warn(
        `[Email] Not connected (${reason}). Banking still works; email notifications will be skipped.`
      );
      if (error.code === 'EAUTH') {
        console.warn(
          '[Email] Fix options:\n' +
          '  1. (Recommended) Use a Gmail App Password:\n' +
          '     → https://myaccount.google.com/apppasswords\n' +
          '     → Set EMAIL_APP_PASSWORD=<your-app-password> in server/.env\n' +
          '     → Remove CLIENT_ID, CLIENT_SECRET, REFRESH_TOKEN\n' +
          '  2. Generate a new OAuth refresh token:\n' +
          '     → https://developers.google.com/oauthplayground\n' +
          '     → Update REFRESH_TOKEN in server/.env\n' +
          '     → Publish your Google Cloud app to "Production" to avoid 7-day expiry'
        );
      }
    } else {
...
async function sendDebitEmail(userEmail, name, amount, transactionId, recipientName, availableBalance, currency = "INR") {
...
Amount Withdrawn   : ${currPrefix}${formattedAmount}
...
async function sendCreditEmail(userEmail, name, amount, transactionId, senderName, availableBalance, currency = "INR") {
...
Amount Received    : ${currPrefix}${formattedAmount}
From               : ${senderName}
Transaction ID     : ${transactionId}
Date & Time        : ${timestamp}
Transaction Status : COMPLETED
